...
export default function findSelectedOptionInXxxOf(validator, rootSchema, schema, fallbackField, xxx, formData = {}, experimental_customMergeAllOf) {
    if (Array.isArray(schema[xxx])) {
        ...
        if (data !== undefined) {
            return xxxOfs.find((xxxOfOption) => deepEquals(get(xxxOfOption, [PROPERTIES_KEY, selectorField, DEFAULT_KEY], get(xxxOfOption, [PROPERTIES_KEY, selectorField, CONST_KEY])), data));
        }
    }
    return undefined;
}
