  ...
  function handleAnyOf(childSchema, source, target) {
    ...
    if (source === void 0 || Array.isArray(source) && source.length === 0 || isObject(source) && Object.keys(source).length === 0) {
      ...
      for (const branch of anyOf) {
        result = omit3(branch, source, result);
      }
      return result;
    }
  ...
  function handleDependencies(childSchema, source, target) {
    ...
    for (const [key, deps] of Object.entries(dependencies)) {
      if (key in source && !Array.isArray(deps)) {
        result = omit3(deps, source, result);
      }
    }
    return result;
  }
...
function replacementForInvalidEnumValue(schema, formValue) {
  ...
  const defaultValue = get15(schema, DEFAULT_KEY, NO_VALUE);
    ...
    keys2.forEach((key) => {
      ...
      if (!oldSchemaTypeForKey || oldSchemaTypeForKey === newSchemaTypeForKey) {
        ...
        if (newSchemaTypeForKey === "object" || newSchemaTypeForKey === "array" && Array.isArray(formValue)) {
          ...
          const newOptionDefault = get15(newKeyedSchema, DEFAULT_KEY, NO_VALUE);
          const oldOptionDefault = get15(oldKeyedSchema, DEFAULT_KEY, NO_VALUE);
          ...
          const newOptionConst = get15(newKeyedSchema, CONST_KEY, NO_VALUE);
          const oldOptionConst = get15(oldKeyedSchema, CONST_KEY, NO_VALUE);
