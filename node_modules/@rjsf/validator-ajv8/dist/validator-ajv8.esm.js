...
// src/validator.ts
import { deepEquals as deepEquals2, ID_KEY as ID_KEY2, ROOT_SCHEMA_PREFIX, withIdRefPrefix, hashForSchema as hashForSchema2 } from "@rjsf/utils";
...
var AJV8Validator = class {
  ...
  rawValidation(schema, formData) {
    ...
    try {
      if (schema[ID_KEY2]) {
        compiledValidator = this.ajv.getSchema(schema[ID_KEY2]);
      }
  ...
  handleSchemaUpdate(rootSchema) {
    ...
    } else if (!deepEquals2(rootSchema, this.ajv.getSchema(rootSchemaId)?.schema)) {
  ...
  isValid(schema, formData, rootSchema) {
    try {
      ...
      const schemaId = schemaWithIdRefPrefix[ID_KEY2] ?? hashForSchema2(schemaWithIdRefPrefix);
      let compiledValidator;
